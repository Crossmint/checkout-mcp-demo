...
- **Amazon** (US only)
   ...
   - Excludes Amazon Fresh and Whole Foods Market items
   - Excludes items with price per unit measurements (ounce, lb, gram)
...
This MCP server is configured to perform purchases with Crossmint credits. However, simply changing the `CHECKOUT_PAYMENT_METHOD` variable to "usdc" in your `.env` enables purchases with stablecoins.
...
6. **Test vs. production**
   Keep the `ENVIRONMENT` variable in your .env file to "test" so you can test orders. Change it to "prod" to complete real orders.
...
1. `search`
   Search for products on Amazon and display your current token balance. Results exclude:
   - Amazon Fresh items
   - Whole Foods Market items
   - Items with price per unit measurements

   Example Prompt:
   > "Search for wireless headphones under $100"

2. `create-order`
   Creates a new order for a specified product. Amazon products are specified by their ASIN.
...
- **Amazon Product Search**: Search for products using SearchAPI.io with smart filtering
- **Crossmint Integration**: Direct order creation and payment processing using Crossmint's API
...
### 1. `search`
- **Description:** Search for products on Amazon and display your current token balance
- **Parameters:**
...
- **Filtering:**
  - Excludes Amazon Fresh items
  - Excludes Whole Foods Market items
  - Excludes items with price per unit measurements
...
- **Logic:**
  ...
  - If sufficient, creates the order directly with Crossmint and returns order details
